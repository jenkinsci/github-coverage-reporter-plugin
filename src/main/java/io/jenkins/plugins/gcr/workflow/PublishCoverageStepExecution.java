...
public class PublishCoverageStepExecution extends AbstractSynchronousStepExecution<Boolean> {
    ...
    protected Boolean run() throws Exception {
        ...
        boolean res = GithubCoveragePublisher.publishCoverage(build, ws, listener, env, step.getFilePath(), step.getCoverageType(), step.getComparisonOption(), step.getCoverageRateType(),codeCoverageAchieved);
        if (!res) {
